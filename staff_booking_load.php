...
while ($row = $sql->fetch(PDO::FETCH_NUM, PDO::FETCH_ORI_NEXT)) {
  ...
  echo "
   ...
   if($row[6] == 0){//pending -> display accept/reject
     echo"
       <td style='padding:0px;'><a class='actionedt' href='accept-booking.php?id=$id'>Accept</a>
        <a class='actiondlte' href='reject_booking.php?id=$id' OnClick=\"return confirm('Are you sure you want to reject this booking?');\">Reject</a></td></tr>";
   }else{//approved or rejected -> display no action available
     echo"
       <td style='padding:0px; color:red;'>No action available</td></tr>";
   }
